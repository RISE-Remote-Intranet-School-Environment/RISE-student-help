...
class UnitTestUpdateReaction {
    ...
    public void unitTestPutReactionContent() throws Exception {
        ...
        MvcResult result = this.mockMvc.perform((put("http://localhost:" + port + "/posts/" + IdPost + "/reactions")
                        .content(json)
        ...
        MvcResult resalt = mockMvc.perform(get("http://localhost:" + port + "/posts/" + IdPost + "/reactions"))
                .andReturn();
        /*
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.value", is(-1)));*/

        MockHttpServletResponse response = resalt.getResponse();
        JsonParser springParser = JsonParserFactory.getJsonParser();

        List<Object> responseList = springParser.parseList(response.getContentAsString());
        int ray = (int) ((Map<String, Object>) responseList.get(0)).get("value");
        Assert.assertEquals(ray,-1);
    }
}
